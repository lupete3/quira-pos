<div class="d-flex flex-column flex-lg-row gap-4 mb-4" style="height: 90vh;">
    ...
    <section class="flex-fill d-flex flex-column border rounded shadow-sm bg-white p-3" style="max-width: 720px;">
        <header class="mb-3">
            <div class="d-flex flex-wrap gap-2 justify-content-between align-items-center">
                <h4 class="m-0"><i class="bx bx-box fs-3 me-2"></i> {{ __('pos.produits') }}</h4>
            ...
            <div class="d-flex flex-wrap gap-2 mt-2">
                <input type="search" class="form-control w-auto flex-grow-1" placeholder="{{ __('pos.rechercher') }}"
        ...
        <div class="flex-grow-1 overflow-none" style="min-height: 400px;">
            @if (!empty($products))
                <div class="row g-3">
                                ...
                                <div class="card-body text-center p-2 d-flex flex-column justify-content-between">
                                    <div>
                                        <h6 class="fw-bold text-truncate" title="{{ $product->name }}">
                                            {{ $product->name }}</h6>
                                        <small class="text-muted d-block">{{ __('pos.ref') }}:
                                            {{ $product->reference }}</small>
                                        <small class="text-muted">
                                            {{ __('pos.stock') }}:
                                            {{ $product->stores()->where('store_id', Auth::user()->stores()->first()->id)->first()->pivot->quantity }}
                                        </small>
                                    </div>
                                    <span class="btn btn-outline-primary fw-bold fs-6 mt-2">
                    ...
                    @empty
                        <div class="text-center text-muted p-3">{{ __('pos.aucun_produit') }}</div>
                    @endforelse
                </div>
    ...
    <section class="flex-fill d-flex flex-column border rounded shadow-sm bg-white p-3 mx-lg-3">
        <header class="mb-3 d-flex justify-content-between align-items-center">
            <h4 class="m-0"><i class="bx bx-shopping-bag fs-3 me-2"></i> {{ __('pos.panier_facture') }}</h4>
        </header>
                        ...
                        <div wire:key="{{ $index }}" class="card shadow-sm p-0">
                            <div class="card-body d-flex justify-content-between align-items-center">
                                <div>
                                    ...
                                    <small class="text-muted">{{ __('pos.prix') }} :
                                        {{ number_format($item['price'], 2) }} {{ company()?->devise }}</small>
                                </div>
                                <div class="d-flex align-items-center gap-3">
                                    ...
                                    <div class="fw-semibold fs-12">{{ number_format($item['subtotal'], 2) }}
                                        {{ company()?->devise }}</div>
                                    <button class="btn btn-outline-danger btn-sm"
                                        wire:click="removeItem({{ $index }})"
                                        aria-label="{{ __('pos.supprimer') }}">
        ...
        <div class="d-flex flex-column gap-2 fs-5">
            ...
            <div class="d-flex justify-content-between align-items-center">
                <label for="discount" class="mb-0">{{ __('pos.reduction') }}</label>
            ...
            <div class="d-flex justify-content-between fw-bold fs-4">
                <div class="m-2">
                    <label for="client_id" class="form-label fw-semibold">{{ __('pos.client') }}</label>
                ...
                <div class="m-2">
                    <label for="total_paid" class="form-label fw-semibold">{{ __('pos.montant_paye') }}</label>
            ...
            <div class="d-flex justify-content-between gap-2 mt-auto">
                <button class="btn btn-success btn-lg" wire:click="saveSale" wire:loading.attr="disabled"
                    ...
                    <i class="bx bx-check me-2"></i> {{ __('pos.valider_vente') }}
                </button>
                <button class="btn btn-outline-danger btn-lg" wire:click="clearCart"
                    @if (!$cart) disabled @endif>
                    <span wire:loading class="spinner-border spinner-border-sm me-2" role="status"></span>
                    <i class="bx bx-x me-2"></i> {{ __('pos.annuler') }}
                </button>
            </div>
        </div>
