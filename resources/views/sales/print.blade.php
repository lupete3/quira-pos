...
<body>
  ...
  @php
    ...
    $sumItems = $sale->items->sum('total_price'); // total sans réduction
    $discount = $sumItems - ($sale->total_amount);
  @endphp
  ...
  <div class="item-total">
    <span>{{ __('facture.remaining') }} :</span>
    {{ round(($sale->total_amount - $sale->total_paid + ($sale->discount ?? 0)), 2) }}{{ company()->devise ?? '' }}
  </div>
